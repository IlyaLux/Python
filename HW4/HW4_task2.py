...
import random
my_list = [random.randint(1, 10) for _ in range(int(input("Введите количество кустов: ")))]
print(my_list)

new_list = list()
for i in range(-1,len(my_list)-1):
    new_list.append(my_list[i-1] + my_list[i] + my_list[i+1])
    print(f'{i-1} куст + {i} куст + {i+1} куст = {my_list[i-1] + my_list[i] + my_list[i+1]} ягод')
print(f'Максимум {max(new_list)} ягод за раз')
